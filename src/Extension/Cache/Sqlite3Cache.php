 ...
 *       $getID3 = new getID3_cached_sqlite3($table='getid3_cache',
 * $hide=FALSE);
 *       $getID3->encoding = 'UTF-8';
 *       $info1 = $getID3->analyze('file1.flac');
 *       $info2 = $getID3->analyze('file2.wv');
 ...
 * ***  set $hide to TRUE to prefix db file with .ht to pervent access from web
 * client
 ...
 * there is a plan to add directory scanning and analyzing to make things work
 * much faster
 */
...
class Sqlite3Cache extends GetId3
{
    ...
    public function __construct(
      ...
      ?bool $hide = false
        ...
        if ($hide) {
            $file = dirname(__FILE__).'/.ht.'.basename(__FILE__,
                'php').'sqlite';
        }
     ...
     */
    private function clear_cache(): SQLite3Result
    {
        $db = $this->db;
     ...
     * this is almost the same as MySQL, with the exception of the dirname
     * being added
     *
     * @return bool
     */
     ...
     * requesting multiple files which is ideal for podcasting, playlists, etc.
     *
     * @param string $dir directory to search the cache database for
     *
     * @return array return an array of matching id3 data
     */
    public function get_cached_dir(string $dir): array
    {
        $db = $this->db;
        $rows = [];
