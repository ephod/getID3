...
class GetId3
{
    ...
    /**
     * CASE SENSITIVE! - i.e. (must be supported by iconv()). Examples:
     * ISO-8859-1  UTF-8  UTF-16  UTF-16BE
     *
     * @var string
     */
    ...
    /**
     * Should always be 'ISO-8859-1', but some tags may be written in other
     * encodings such as 'EUC-CN' or 'CP1252'
     *
     * @var string
     */
    ...
    /**
     * Copy tags to root key 'tags_html' properly translated from various
     * encodings to HTML entities
     *
     * @var bool
     */
    ...
    /**
     * Check whether file is larger than 2GB and thus not supported by 32-bit
     * PHP (null: auto-detect based on PHP_INT_MAX)
     *
     * @var bool|null
     */
      ...
      ?string $original_filename = ''
        ...
        try {
            ...
            if (!$this->option_tag_id3v2) {
                ...
                if ((substr($header, 0,
                      3) == 'ID3') && (strlen($header) == 10)) {
                    $this->info['id3v2']['header'] = true;
                    $this->info['id3v2']['majorversion'] = ord($header{3});
                    $this->info['id3v2']['minorversion'] = ord($header{4});
                    $this->info['avdataoffset'] += getid3_lib::BigEndian2Int(substr($header,
                        6, 4),
            ...
            if (!empty($determined_format['iconv_req']) && !function_exists('mb_convert_encoding') && !function_exists('iconv') && !in_array($this->encoding,
    ...
    public function GetFileFormat(string &$filedata, ?string $filename = '')
    {
        ...
        foreach ($this->GetFileFormatArray() as $format_name => $info) {
            ...
            if (!empty($info['pattern']) && preg_match('#'.$info['pattern'].'#s',
                $filedata)) {
        ...
        if (preg_match('#\\.mp[123a]$#i', $filename)) {
            ...
            return $info;
        } elseif (preg_match('#\\.cue$#i',
            $filename) && preg_match('#FILE "[^"]+" (BINARY|MOTOROLA|AIFF|WAVE|MP3)#',
            $filedata)) {
    ...
    public function CharConvert(array &$array, string $encoding): void
    {
        ...
        foreach ($array as $key => $value) {
            ...
            elseif (is_string($value)) {
                $array[$key] = trim(getid3_lib::iconv_fallback($encoding,
                  $this->encoding, $value));
            }
        }
    }
        ...
        if (!empty($this->info['fileformat']) && !empty($this->info['dataformat']) && ($this->info['fileformat'] == 'ogg') && ($this->info['audio']['dataformat'] == 'vorbis')) {
            ...
            if (!empty($this->info['avdataoffset']) || (isset($this->info['avdataend']) && ($this->info['avdataend'] < $this->info['filesize']))) {
                ...
                $this->info[$algorithm.'_data'] = getid3_lib::hash_data($this->info['filenamepath'],
                  $this->info['avdataoffset'], $this->info['avdataend'],
                  $algorithm);
