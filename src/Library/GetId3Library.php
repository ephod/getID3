...
namespace GetId3\Library;

/**
 * Class GetId3Library
 *
 * @package GetId3\Library
 */
class GetId3Library
{
      ...
      ?string $htmlencoding = 'UTF-8'
        ...
        for ($i = 0; $i < strlen($string); $i++) {
            if ($hex) {
                ...
                $returnstring .= ' '.(preg_match("#[\x20-\x7E]#",
                    $string{$i}) ? $string{$i} : '¤');
            }
            if ($spaces) {
                $returnstring .= ' ';
     ...
     */
    public static function trunc(float $floatnumber)
    {
        if ($floatnumber >= 1) {
    ...
    public static function intValueSupported(int $num): bool
    {
        ...
        if ($hasINT64 === null) { // 10x faster than is_null()
            $hasINT64 = is_int(pow(2,
              31)); // 32-bit int are limited to (2^31)-1
      ...
      ?int $maxbits = 52
        ...
        while (($binarypointnumber{0} != '1') || (substr($binarypointnumber, 1,
            ...
            if (substr($binarypointnumber, 1, 1) == '.') {
                $exponent--;
                $binarypointnumber = substr($binarypointnumber, 2,
                    1).'.'.substr($binarypointnumber, 3);
            } else {
        ...
        $binarypointnumber = str_pad(substr($binarypointnumber, 0,
          $maxbits + 2), $maxbits + 2, '0', STR_PAD_RIGHT);
     ...
     * @return string|false
     */
    public static function Float2String(float $floatvalue, int $bits)
    {
        $exponentbits = 0;
        $fractionbits = 0;
        switch ($bits) {
        ...
        $normalizedbinary = self::NormalizeBinaryPoint(self::Float2BinaryDecimal($floatvalue),
          $fractionbits);
        $biasedexponent = pow(2,
            $exponentbits - 1) - 1 + $normalizedbinary['exponent']; // (127 or 1023) +/- exponent
     ...
     * @return float|false
     */
    public static function LittleEndian2Float(string $byteword)
    {
        return self::BigEndian2Float(strrev($byteword));
    }

    /**
     * ANSI/IEEE Standard 754-1985, Standard for Binary Floating Point
     * Arithmetic
     *
     ...
     * @return float|false
     */
    public static function BigEndian2Float(string $byteword)
    {
        ...
        if (($exponent == (pow(2, $exponentbits) - 1)) && ($fraction != 0)) {
            // Not a Number
            $floatvalue = false;
        } elseif (($exponent == (pow(2,
                $exponentbits) - 1)) && ($fraction == 0)) {
            if ($signbit == '1') {
                $floatvalue = '-infinity';
            } else {
                $floatvalue = '+infinity';
            ...
            $floatvalue = pow(2, (-1 * (pow(2,
                    $exponentbits - 1) - 2))) * self::DecimalBinary2Float($fractionstring);
            if ($signbit == '1') {
                $floatvalue *= -1;
            }
        } elseif ($exponent != 0) {
            $floatvalue = pow(2, ($exponent - (pow(2,
                    $exponentbits - 1) - 1))) * (1 + self::DecimalBinary2Float($fractionstring));
            if ($signbit == '1') {
                $floatvalue *= -1;
            }
        }
     ...
     * @return int|float|false
     */
    public static function LittleEndian2Int(
      string $byteword,
      ?bool $signed = false
    ) {
     ...
     * @return string
     */
    public static function LittleEndian2Bin(string $byteword): string
    {
     ...
     * @return int|float
     */
    public static function Bin2Dec(string $binstring, ?bool $signed = false)
    {
        $signmult = 1;
        if ($signed) {
            if ($binstring{0} == '1') {
    ...
    public static function Bin2String(string $binstring): string
    {
        ...
        for ($i = 0; $i < strlen($binstringreversed); $i += 8) {
            $string = chr(self::Bin2Dec(strrev(substr($binstringreversed, $i,
                8)))).$string;
        }
     ...
     * @return array|false
     */
    public static function array_merge_clobber(array $array1, array $array2)
    {
     ...
     * @return array|false
     */
    public static function array_merge_noclobber(array $array1, array $array2)
    {
        if (!is_array($array1) || !is_array($array2)) {
            return false;
        }
     ...
     */
    public static function flipped_array_merge_noclobber(
      array $array1,
      array $array2
    ) {
        if (!is_array($array1) || !is_array($array2)) {
    ...
    public static function PlaytimeString(int $seconds): string
    {
        ...
        return $sign.($H ? $H.':' : '').($H ? str_pad($M, 2, '0',
            STR_PAD_LEFT) : intval($M)).':'.str_pad($S, 2, 0, STR_PAD_LEFT);
    }
     ...
     * @return int|float
     */
    public static function DateMac2Unix(int $macdate)
    {
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0,
            1)) + (float)(self::BigEndian2Int(substr($rawdata, 1, 1)) / pow(2,
              8));
    }
    ...
    {
        return self::BigEndian2Int(substr($rawdata, 0,
            2)) + (float)(self::BigEndian2Int(substr($rawdata, 2, 2)) / pow(2,
              16));
    }
    ...
    public static function FixedPoint2_30(string $rawdata): float
    {
        ...
        return self::Bin2Dec(substr($binarystring, 0,
            2)) + (float)(self::Bin2Dec(substr($binarystring, 2, 30)) / pow(2,
              30));
    }
     ...
     * @return array
     */
    public static function CreateDeepArray(
      string $ArrayPath,
      string $Separator,
      $Value
    ): array {
        ...
        if (($pos = strpos($ArrayPath, $Separator)) !== false) {
            $ReturnedArray[substr($ArrayPath, 0,
              $pos)] = self::CreateDeepArray(substr($ArrayPath, $pos + 1),
              $Separator, $Value);
        } else {
     ...
     * @return int|false
     */
    public static function array_max(array $arraydata, ?bool $returnkey = false)
    {
        $maxvalue = false;
        $maxkey = false;
     ...
     * @return int|false
     */
    public static function array_min(array $arraydata, ?bool $returnkey = false)
    {
        $minvalue = false;
        $minkey = false;
        foreach ($arraydata as $key => $value) {
     ...
     * @return array|false
     */
    public static function XML2array(string $XMLstring)
    {
        if (function_exists('simplexml_load_string') && function_exists('libxml_disable_entity_loader')) {
            // http://websec.io/2012/08/27/Preventing-XEE-in-PHP.html
            // https://core.trac.wordpress.org/changeset/29378
            $loader = libxml_disable_entity_loader(true);
            $XMLobject = simplexml_load_string($XMLstring, 'SimpleXMLElement',
              LIBXML_NOENT);
    ...
    /**
     * self::md5_data() - returns md5sum for a file from startuing position to
     ...
     */
    public static function hash_data(
      string $file,
      int $offset,
      int $end,
      string $algorithm
     ...
     * @return string
     */
    public static function iconv_fallback_utf8_utf16le(
      string $string,
      ?bool $bom = false
    ): string {
        $newcharstring = '';
     ...
     * @throws Exception
     */
    public static function iconv_fallback(
      string $in_charset,
      string $out_charset,
      string $string
    ): string {
        ...
        if (function_exists('mb_convert_encoding')) {
            if ($converted_string = @mb_convert_encoding($string, $out_charset,
              $in_charset)) {
                switch ($out_charset) {
                    case 'ISO-8859-1':
      ...
      ?string $charset = 'ISO-8859-1'
        ...
        if (is_string($data)) {
            ...
            foreach ($data as $key => $value) {
                $return_data[$key] = self::recursiveMultiByteCharString2HTML($value,
                  $charset);
            }
     ...
     * @return float
     */
    public static function RGADadjustmentLookup(
      int $rawadjustment,
      int $signbit
    ): float {
     ...
     * @return array|false
     */
    public static function GetDataImageSize(
      string $imgData,
      ?array &$imageinfo = []
    ) {
        static $tempdir = '';
    ...
    public static function ImageExtFromMime(string $mime_type): string
    {
        ...
        return str_replace(['image/', 'x-', 'jpeg'], ['', '', 'jpg'],
          $mime_type);
    }
    ...
    public static function CopyTagsToComments(array &$ThisFileInfo): bool
    {
        ...
        if (!empty($ThisFileInfo['tags'])) {
            ...
            foreach ($StandardizeFieldNames as $badkey => $goodkey) {
                if (array_key_exists($badkey,
                    $ThisFileInfo['comments']) && !array_key_exists($goodkey,
     ...
     * @return string
     */
    public static function EmbeddedLookup(
      string $key,
      int $begin,
      int $end,
      string $file,
     ...
     * @return string|false
     */
    public static function truepath(string $filename)
    {
     ...
     * @return string
     */
    public static function mb_basename(
      string $path,
      ?string $suffix = null
    ): string {
